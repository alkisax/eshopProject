...
/*
  📄 Native version of News page
  - Fetches /api/posts
  - Filters where subPage.name === 'news'
...
const NewsScreen = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        // ✅ Filter only "news" posts
        const newsPosts = all.filter(
  ...
  if (!loading && posts.length === 0) {
    return (
      <View style={styles.center}>
        <Text style={styles.emptyText}>Δεν βρέθηκαν νέα.</Text>
      </View>
    );
  }
