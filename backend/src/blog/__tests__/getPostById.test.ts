...
beforeAll(async () => {
  ...
  const post = await Post.create({
    content: { time: Date.now(), blocks: [{ type: 'paragraph', data: { text: 'ById' } }], version: '2.28.0' },
    subPage: subPageId,
    pinned: false,
  });
...
describe('GET /api/posts/:postId', () => {
  it('returns one post (200)', async () => {
    const res = await request(app).get(`/api/posts/${postId}`).set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data._id).toBe(postId);
  });

  it('404 not found', async () => {
    const res = await request(app).get('/api/posts/68ad7e285d9e6a24a76b249e').set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(404);
  });

  it('500 invalid id format', async () => {
    const res = await request(app).get('/api/posts/not-an-id').set('Authorization', `Bearer ${adminToken}`);
...
describe('DAO errors with spyOn', () => {
  it('500 when DAO throws', async () => {
    const spy = jest.spyOn(postDAO, 'getPostById').mockRejectedValue(new Error('DB fail'));
    const res = await request(app).get(`/api/posts/${postId}`).set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(500);
    spy.mockRestore();
  });
