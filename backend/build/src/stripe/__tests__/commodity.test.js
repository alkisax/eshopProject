...
beforeAll(async () => {
    ...
    // Create admin user
    const hashedPasswordAdmin = await (0, bcrypt_1.hash)(TEST_ADMIN.password, 10);
...
describe('Commodity Controller', () => {
    ...
    it('should fetch all commodities', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get('/api/commodity');
        expect(res.status).toBe(200);
        expect(Array.isArray(res.body.data)).toBe(true);
    });
    it('should fetch a commodity by ID', async () => {
        const res = await (0, supertest_1.default)(app_1.default).get(`/api/commodity/${commodityId}`);
        expect(res.status).toBe(200);
    ...
    it('should sell a commodity and update stock/soldCount (admin only)', async () => {
        ...
        const sellRes = await (0, supertest_1.default)(app_1.default)
    ...
    describe('Delete single comment', () => {
        ...
        beforeEach(async () => {
            ...
            const res = await (0, supertest_1.default)(app_1.default)
                .post(`/api/commodity/${commodityId}/comments`)
                .set('Authorization', `Bearer ${userToken}`)
                .send({ user: userId, text: 'Temp comment', rating: 4 });
            expect(res.status).toBe(200);
        ...
        it('should return 400 if missing commentId (mocked)', async () => {
            jest.spyOn(commodity_dao_1.commodityDAO, 'deleteCommentFromCommoditybyCommentId')
                .mockImplementationOnce(() => { throw new errors_types_1.ValidationError('Missing commentId'); });
            const res = await (0, supertest_1.default)(app_1.default)
    ...
    describe('Negative cases', () => {
        ...
        it('should return 404 when fetching a non-existent commodity', async () => {
            ...
            const res = await (0, supertest_1.default)(app_1.default).get(`/api/commodity/${fakeId}`);
...
describe('some additional sad way tests', () => {
    ...
    beforeAll(async () => {
        ...
        const hashedPassword = await (0, bcrypt_1.hash)('securepassword', 10);
        await users_models_1.default.create({
            username: 'adminuser',
            name: 'Admin User',
            email: 'admin@example.com',
    ...
    it('should return 400 if commodityDAO.createCommodity throws ValidationError (mocked)', async () => {
        // Arrange: spy on DAO
        jest.spyOn(commodity_dao_1.commodityDAO, 'createCommodity')
            .mockImplementationOnce(() => { throw new errors_types_1.ValidationError('Invalid commodity data'); });
        // Act: make request
        const res = await (0, supertest_1.default)(app_1.default)
            .post('/api/commodity')
    ...
    it('should return 400 if DAO throws inside addComment', async () => {
        ...
        const commodityRes = await (0, supertest_1.default)(app_1.default)
            .post('/api/commodity')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({
            name: 'Test Commodity',
        ...
        // 👇 force DAO to throw
        jest.spyOn(commodity_dao_1.commodityDAO, 'addCommentToCommodity')
            .mockImplementationOnce(() => { throw new errors_types_1.ValidationError('Mocked DAO fail'); });
        const res = await (0, supertest_1.default)(app_1.default)
            .post(`/api/commodity/${commodityId}/comments`)
            .set('Authorization', `Bearer ${adminToken}`)
    ...
    it('should return 400 if DAO throws inside clearComments', async () => {
        ...
        const commodityId = commodityRes.body.data._id;
        // 👇 force DAO to throw
        jest.spyOn(commodity_dao_1.commodityDAO, 'clearCommentsFromCommodity')
            .mockImplementationOnce(() => { throw new errors_types_1.ValidationError('Mocked DAO fail on clear'); });
        const res = await (0, supertest_1.default)(app_1.default)
            .delete(`/api/commodity/${commodityId}/comments`)
            .set('Authorization', `Bearer ${adminToken}`);
        expect(res.status).toBe(400);
...
//# sourceMappingURL=commodity.test.js.map