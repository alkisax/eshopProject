...
export const CartActionsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchParticipantId = async (): Promise<string | null> => {
    ...
    if (user) {
      ...
      console.log(
        "setp 1. See if user has participant. user from context: ",
        user
      );
      ...
      console.log(
        "step 4. no existing user. we will create a participant not associated with a user"
      );
  ...
  const removeItemFromCart = async (
    ...
    try {
      ...
      setCartCount(
        updated.data.data.items.reduce((sum, item) => sum + item.quantity, 0)
      );
      console.log(`Removed commodity ${commodityId} completely from cart`);
    } catch (err: unknown) {
      if (axios.isAxiosError(err)) {
        console.error(
          "Error removing commodity:",
          err.response?.data || err.message
        );
      } else {
        console.error("Unexpected error:", err);
